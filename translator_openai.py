        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "당신은 넷플릭스, 디즈니+ 등에서 근무하는 전문 영상 자막 번역가입니다. 자연스러운 일상 대화체로 축약하며, 긴 문장은 핵심만 살려 짧게 번역합니다."
                    },
                    {
                        "role": "user",
                        "content": prompt
...
3. 직역 금지, 자연스러운 일상 대화처럼 의역
4. 전후 맥락을 고려한 일관된 번역
5. 불필요한 표현 최대한 제거하고 임팩트 있게
6. 각 번역은 "[번호]" 형식으로 구분
...
한국어 번역:"""
                    ...
                    response = self.client.chat.completions.create(
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": "당신은 전문 영상 자막 번역가입니다. 연속된 자막의 문맥을 파악하여 일관성 있게 번역하되, 자연스러운 대화체로 축약하고 긴 문장은 핵심만 살립니다."
                            },
                            {
                                "role": "user",
